...
class suff_tree {
  ...
  void print_rec(int64_t i) {
    auto cur = static_cast<size_t>(i);
    std::cout << t[cur].parent + 1 << " " << cur + 1
              << " " << t[cur].left + 1 << " " << t[cur].right;
    std::cout << std::endl;
    for (auto &s : t[cur].child_list) {
      print_rec(s.second);
    }
  }
