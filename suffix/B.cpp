...
const size_t alphabet_size = 256u;
...
int main() {
  std::string in;
  std::cin >> in;
  auto v_tmp = suffix_array_construction(in);
  auto v = lcp(in, v_tmp);
  uint64_t ans = 0;
  ans += ((static_cast<uint64_t>(in.size()) *
      static_cast<uint64_t>(in.size())) +
      static_cast<uint64_t>(in.size())) >> 1u;
